...
pub fn platform_rules_prompt() -> String {
    [
        "你是多角色协作任务中的一个成员。",
        "你只能完成当前角色职责范围内的工作。",
        "你不能自行切换角色。",
        "当你认为下一阶段应由其他角色继续时，可以输出 HANDOFF 块提出建议。",
...
pub fn build_role_system_prompt(
    ...
    sections.push(platform_rules_prompt());
    ...
    if let Some(override_prompt) = system_prompt_override.map(str::trim).filter(|value| !value.is_empty()) {
        sections.push(format!("额外系统提示覆盖：\n{}", override_prompt));
    }
...
mod tests {
    ...
    #[test]
    fn build_role_system_prompt_includes_archetype_and_override_layers() {
        ...
        assert!(prompt.contains("额外系统提示覆盖：\n必须输出明确的交接建议"));
    }
}
